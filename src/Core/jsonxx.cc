...
Object::Object(Object&& other) : odd(std::move(other.odd)) {
  value_map_ = std::move(other.value_map_);
}
Object& Object::operator=(Object&& other) {
  if (this != &other) {
    odd = std::move(other.odd);
    reset();
    value_map_ = std::move(other.value_map_);
  }
  return *this;
...
bool ReadString(jsonxx::Object const& json, char const* key, std::string& outVal) {
    static const std::string invalidStr = Format("Invalid%d", std::time(nullptr));
    std::string result = json.get<jsonxx::String>(key, invalidStr);
    if (result != invalidStr) {
