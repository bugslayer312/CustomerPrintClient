...
const jsonxx::Number InvalidNumber =  std::numeric_limits<jsonxx::Number>::quiet_NaN();

template<class T>
bool ReadNumber(jsonxx::Object const& json, char const* key, T& outVal) {
    jsonxx::Number result = json.get<jsonxx::Number>(key, InvalidNumber);
    if (result != InvalidNumber) {
        outVal = static_cast<T>(result);
        return true;
    }
    return false;
}
